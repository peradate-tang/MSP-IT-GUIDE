...
async onModuleInit() {
  ...
  if (count === 0) {
    const categoryRepo = this.articlesRepository.manager.getRepository('categories');
    let retries = 10;
    let categories = [];
    while (retries > 0) {
  ...
  async findAll(query: { search?: string; categoryId?: number; status?: string; page?: number; limit?: number }) {
  ...
  async create(data: Partial<Article> & { title: string; content: string }, authorId: number): Promise<Article> {
